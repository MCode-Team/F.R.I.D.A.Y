...
If you discover a security vulnerability in friday, please report it by:
...
chmod 600 ~/.friday/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.friday/config.json` with file permissions set to `0600`
...
- ✅ Run friday with a dedicated user account
...
- Keep authentication data in `~/.friday/whatsapp-auth` secure (mode 0700)
...
- Subscribe to security advisories for friday and its dependencies
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.friday
   chmod 600 ~/.friday/config.json
   chmod 700 ~/.friday/whatsapp-auth
   ```
...
- **Chat history is stored locally** - protect the `~/.friday` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.friday/logs/friday.log
   ```
...
- GitHub Security Advisories: https://github.com/HKUDS/friday/security/advisories
- Release Notes: https://github.com/HKUDS/friday/releases
